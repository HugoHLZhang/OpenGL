//
// OPENGL 202 - Dragon + illumination de lambert
// 
...
struct Application
{
    ...
    bool Initialize()
    {
        ...
        m_textureShader.LoadFragmentShader("texture_lambert.fs.glsl");
...
//-x  <--+--> +x
    ...
    void Render()
    {
        ...
        // Lambert --
        // L'algorithme d'illumination necessite la position de (ou direction VERS) la lumiere
...
int main(void)
{
    ...
    window = glfwCreateWindow(1280, 720, "OpenGL 202 - Lambert", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
        return -1;
