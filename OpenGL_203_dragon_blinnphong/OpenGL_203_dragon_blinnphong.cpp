//
// OPENGL 203 Dragon + illumination de Blinn-Phong
// 
...
struct Application
{
    ...
    bool Initialize()
    {
        ...
        m_textureShader.LoadFragmentShader("texture_blinnphong.fs.glsl");
...
//-x  <--+--> +x
    ...
    void Render()
    {
        ...
        const int32_t SHININESS = glGetUniformLocation(program, "u_Shininess");
...
int main(void)
{
    ...
    window = glfwCreateWindow(1280, 720, "OpenGL 203 - Blinn Phong", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
        return -1;
